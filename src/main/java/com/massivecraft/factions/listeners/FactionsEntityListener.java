...
public class FactionsEntityListener extends AbstractListener {
  ...
  private static final Set<PotionEffectType> badPotionEffects = new LinkedHashSet<>(Arrays
      .asList(PotionEffectType.BLINDNESS, PotionEffectType.CONFUSION, PotionEffectType.HARM,
  ...
  /**
   * Who can I hurt? I can never hurt members or allies. I can always hurt enemies. I can hurt neutrals
   * as long as
  ...
  public void onEntityDamage(EntityDamageEvent event) {
    ...
    } else if (FactionsPlugin.getInstance().conf().factions().protection()
        .isSafeZonePreventAllDamageToPlayers() && isPlayerInSafeZone(event.getEntity())) {
      // Players can not take any damage in a Safe Zone
      event.setCancelled(true);
    } else if (event.getCause() == EntityDamageEvent.DamageCause.FALL && event
  ...
  // mainly for flaming arrows; don't want allies or people in safe zones to be ignited even after damage
  // event is cancelled
  @EventHandler(priority = EventPriority.NORMAL, ignoreCancelled = true)
  ...
  public void onPotionSplashEvent(PotionSplashEvent event) {
    ...
    for (LivingEntity target : event.getAffectedEntities()) {
      EntityDamageByEntityEvent sub =
          new EntityDamageByEntityEvent((Entity) thrower, target, EntityDamageEvent.DamageCause.CUSTOM, 0);
      if (!this.canDamagerHurtDamagee(sub, true)) {
        event.setIntensity(target,
            0.0);  // affected entity list doesn't accept modification (so no iter.remove()), but this works
      }
    }
  }
  ...
  public boolean canDamagerHurtDamagee(EntityDamageByEntityEvent sub, boolean notify) {
    ...
    // for damage caused by projectiles, getDamager() returns the projectile... what we need to know is the
    // source
    if (damager instanceof Projectile) {
      Projectile projectile = (Projectile) damager;
      ...
      if (damager instanceof Player) {
        if (notify) {
          ...
          attacker.msg(TL.PLAYER_CANTHURT,
              (defLocFaction.isSafeZone() ? TL.REGION_SAFEZONE.toString() : TL.REGION_PEACEFUL.toString()));
        }
        return false;
      }
    ...
    // so we know from above that the defender isn't in a safezone... what about the attacker, sneaky dog
    // that he might be?
  ...
  public void onCreatureSpawn(CreatureSpawnEvent event) {
    ...
    if (faction.isNormal()) {
      if (faction.isPeaceful() && FactionsPlugin.getInstance().conf().factions().specialCase()
          .isPeacefulTerritoryDisableMonsters()) {
        if (event.getEntity() instanceof Monster) {
          event.setCancelled(true);
        }
      }
      if (spawning.getPreventInTerritory().contains(reason) && !spawning.getPreventInTerritoryExceptions()
          .contains(type)) {
        event.setCancelled(true);
      }
    } else if (faction.isSafeZone()) {
      if (spawning.getPreventInSafezone().contains(reason) && !spawning.getPreventInSafezoneExceptions()
          .contains(type)) {
        event.setCancelled(true);
      }
    } else if (faction.isWarZone()) {
      if (spawning.getPreventInWarzone().contains(reason) && !spawning.getPreventInWarzoneExceptions()
          .contains(type)) {
        event.setCancelled(true);
      }
    } else if (faction.isWilderness()) {
      if (spawning.getPreventInWilderness().contains(reason) && !spawning.getPreventInWildernessExceptions()
          .contains(type)) {
        event.setCancelled(true);
      }
    }
  ...
  public void onEntityTarget(EntityTargetEvent event) {
    ...
    if (event.getEntity() instanceof Monster && Board.getInstance()
        .getFactionAt(new FLocation(target.getLocation())).noMonstersInTerritory()) {
      event.setCancelled(true);
    }
  }
  ...
  public void onPaintingBreak(HangingBreakEvent event) {
    ...
    if (event.getCause() == RemoveCause.EXPLOSION) {
      ...
      if ((faction.isWilderness() && !protection.getWorldsNoWildernessProtection()
          .contains(loc.getWorld().getName()) && (protection.isWildernessBlockCreepers() || protection
          .isWildernessBlockFireballs() || protection.isWildernessBlockTNT())) ||
          (faction.isNormal() && (online ? (protection.isTerritoryBlockCreepers() || protection
              .isTerritoryBlockFireballs() || protection.isTerritoryBlockTNT()) :
                                  (protection.isTerritoryBlockCreepersWhenOffline() || protection
                                      .isTerritoryBlockFireballsWhenOffline() || protection
                                      .isTerritoryBlockTNTWhenOffline()))) ||
          (faction.isWarZone() && (protection.isWarZoneBlockCreepers() || protection
              .isWarZoneBlockFireballs() || protection.isWarZoneBlockTNT())) ||
          faction.isSafeZone()) {
    ...
    if (!FactionsBlockListener.playerCanBuildDestroyBlock((Player) breaker, event.getEntity().getLocation(),
        PermissibleAction.DESTROY, false)) {
      event.setCancelled(true);
    }
  }
  ...
  public void onPaintingPlace(HangingPlaceEvent event) {
    ...
    if (!FactionsBlockListener.playerCanBuildDestroyBlock(event.getPlayer(),
        event.getBlock().getRelative(event.getBlockFace()).getLocation(), PermissibleAction.BUILD, false)) {
      event.setCancelled(true);
    }
  }
  ...
  public void onEntityChangeBlock(EntityChangeBlockEvent event) {
    ...
    if (entity instanceof Enderman) {
      ...
      Faction faction = Board.getInstance().getFactionAt(new FLocation(loc));
      MainConfig.Factions.Protection protection = FactionsPlugin.getInstance().conf().factions().protection();
      // it's a bit crude just using fireball protection, but I'd rather not add in a whole new set of
      // xxxBlockWitherExplosion or whatever
      if ((faction.isWilderness() && protection.isWildernessBlockFireballs() && !protection
          .getWorldsNoWildernessProtection().contains(loc.getWorld().getName())) ||
          (faction.isNormal() && (faction.hasPlayersOnline() ? protection.isTerritoryBlockFireballs() :
  ...
  private boolean stopEndermanBlockManipulation(Location loc) {
    ...
    if (claimFaction.isWilderness()) {
      ...
      return claimFaction.hasPlayersOnline() ? protection.isTerritoryDenyEndermanBlocks() :
             protection.isTerritoryDenyEndermanBlocksWhenOffline();
