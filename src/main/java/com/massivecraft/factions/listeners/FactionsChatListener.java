...
public class FactionsChatListener implements Listener {
  ...
  // this is for handling slashless command usage and faction/alliance chat, set at lowest priority so
  // Factions gets to them first
  ...
  public void onPlayerEarlyChat(AsyncPlayerChatEvent event) {
    ...
    if (chat == ChatMode.MOD) {
      ...
      String message =
        String.format(chatConf.getModChatFormat(), ChatColor.stripColor(me.getNameAndTag()), msg);
      ...
      FactionsPlugin.getInstance()
        .log(Level.INFO, ChatColor.stripColor("ModChat " + myFaction.getTag() + ": " + message));
      ...
      FactionsPlugin.getInstance()
        .log(Level.INFO, ChatColor.stripColor("FactionChat " + myFaction.getTag() + ": " + message));
      ...
      String message =
        String.format(chatConf.getAllianceChatFormat(), ChatColor.stripColor(me.getNameAndTag()), msg);
      ...
      String message =
        String.format(chatConf.getTruceChatFormat(), ChatColor.stripColor(me.getNameAndTag()), msg);
  ...
  // this is for handling insertion of the player's faction tag, set at highest priority to give other
  // plugins a chance to modify chat first
  ...
  public void onPlayerChat(AsyncPlayerChatEvent event) {
    ...
    if (!chatConf.getTagReplaceString().isEmpty() && eventFormat.contains(chatConf.getTagReplaceString())) {
      ...
      padBefore = false;
      padAfter = false;
    } else if (!chatConf.getTagInsertAfterString().isEmpty() && eventFormat
      .contains(chatConf.getTagInsertAfterString())) {
      // we're using the "insert after string" method
      InsertIndex =
        eventFormat.indexOf(chatConf.getTagInsertAfterString()) + chatConf.getTagInsertAfterString().length();
    } else if (!chatConf.getTagInsertBeforeString().isEmpty() && eventFormat
      .contains(chatConf.getTagInsertBeforeString())) {
    ...
    String formatStart =
      eventFormat.substring(0, InsertIndex) + ((padBefore && !me.getChatTag().isEmpty()) ? " " : "");
    String formatEnd =
      ((padAfter && !me.getChatTag().isEmpty()) ? " " : "") + eventFormat.substring(InsertIndex);
    ...
    if (chatConf.isTagRelationColored()) {
      for (Player listeningPlayer : event.getRecipients()) {
        ...
        try {
          ...
          FactionsPlugin.getInstance()
            .log(Level.SEVERE, "NOTE: This can be fixed right now by setting chat tagInsertIndex to 0.");
          return;
        }
      }
