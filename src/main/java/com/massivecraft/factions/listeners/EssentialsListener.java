...
public class EssentialsListener implements Listener {
  ...
  public void onLeave(FPlayerLeaveEvent event) throws Exception {
    ...
    if (user == null) {
      FactionsPlugin.getInstance().log(Level.WARNING,
          "Attempted to remove Essentials homes for " + event.getfPlayer().getName() + " " +
              "but no Essentials data at all was found for this user. " +
              "This may be a bug in Essentials, or may be that the player only played prior to adding " +
              "Essentials to the server");
      return;
    }
    ...
    for (String homeName : user.getHomes()) {
      ...
      try {
        ...
        FactionsPlugin.getInstance().getLogger().warning(
            "Tried to check on home \"" + homeName + "\" for user \"" + event.getfPlayer()
                .getName() + "\" but Essentials said world \"" + e
                .getWorld() + "\" does not exist. Skipping it.");
        continue;
      }
      ...
      if (factionAt.equals(faction) && factionAt.isNormal()) {
        ...
        FactionsPlugin.getInstance()
            .log(Level.INFO, "FactionLeaveEvent: Removing home %s, player %s, in territory of %s",
