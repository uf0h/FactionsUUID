...
public class Loader {
  ...
  public static HoconConfigurationLoader getLoader(String file) {
    ...
    return HoconConfigurationLoader.builder()
        ...
        .setRenderOptions(
            ConfigRenderOptions.defaults().setComments(true).setOriginComments(false).setJson(false))
        .build();
  }

  public static void loadAndSave(HoconConfigurationLoader loader, Object config)
  ...
  public static void load(HoconConfigurationLoader loader, Object config)
      throws IOException, IllegalAccessException {
    CommentedConfigurationNode node = loader.load();
  ...
  private static void loadNode(CommentedConfigurationNode current, CommentedConfigurationNode newNode,
    ...
    for (Field field : getFields(object.getClass())) {
      ...
      String confName =
          configName == null || configName.value().isEmpty() ? field.getName() : configName.value();
      ...
      if (types.contains(field.getType())) {
        if (needsValue) {
          ...
          try {
            if (Set.class.isAssignableFrom(field.getType()) && List.class
                .isAssignableFrom(curNode.getValue().getClass())) {
            ...
            System.out.println(
                "Found incorrect type for " + getNodeName(curNode.getPath()) + ": Expected " + field
                    .getType() + ", found " + curNode.getValue().getClass());
