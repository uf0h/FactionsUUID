...
public class Econ {
  ...
  public static void sendBalanceInfo(FPlayer to, EconomyParticipator about) {
    ...
    to.msg(TL.ECON_BALANCE, about.describeTo(to, true),
  ...
  public static void sendBalanceInfo(CommandSender to, Faction about) {
    ...
    to.sendMessage(ChatColor.stripColor(String.format(TL.ECON_BALANCE.toString(), about.getTag(),
        Econ.moneyString(econ.getBalance(about.getAccountId())))));
  }
  ...
  public static boolean canIControlYou(EconomyParticipator i, EconomyParticipator you) {
    ...
    if (you instanceof Faction && fI == fYou && (FactionsPlugin.getInstance().conf().economy()
        .isBankMembersCanWithdraw() || ((FPlayer) i).getRole().value >= Role.MODERATOR.value)) {
      return true;
    }
...
//			be for losses
    ...
    if (isUUID(ep.getAccountId())) {
      ...
      acc = ep instanceof Faction ? ((Faction) ep).getOfflinePlayer() :
            Bukkit.getOfflinePlayer(ep.getAccountId());
    }
  ...
  public static double calculateClaimCost(int ownedLand, boolean takingFromAnotherFaction) {
    ...
    return FactionsPlugin.getInstance().conf().economy().getCostClaimWilderness() + (FactionsPlugin
        .getInstance().conf().economy().getCostClaimWilderness() * FactionsPlugin.getInstance().conf()
        .economy().getClaimAdditionalMultiplier() * ownedLand) - (takingFromAnotherFaction ?
                                                                  FactionsPlugin.getInstance().conf()
                                                                      .economy()
                                                                      .getCostClaimFromFactionBonus() : 0);
  }
