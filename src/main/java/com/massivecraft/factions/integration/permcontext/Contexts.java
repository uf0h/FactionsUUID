...
public enum Contexts implements Context {
  TERRITORY_RELATION((player) ->
                       FPlayers.getInstance().getByPlayer(player)
                         .getRelationTo(Board.getInstance().getFactionAt(new FLocation(player.getLocation())))
                         .getNameInASet(),
                     Arrays.stream(Relation.values()).map(relation -> relation.name().toLowerCase())
  ...
  ROLE_AT_LEAST((player) ->
                ...
                Arrays.stream(Role.values()).map(role -> role.name().toLowerCase())
                  .collect(Collectors.toSet())),
  ROLE_AT_MOST((player) ->
               {
                 FPlayer p = FPlayers.getInstance().getByPlayer(player);
