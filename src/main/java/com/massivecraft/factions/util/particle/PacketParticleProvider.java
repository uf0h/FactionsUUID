...
public class PacketParticleProvider implements ParticleProvider<ParticleEffect> {
  ...
  @Override
  public void spawn(ParticleEffect particleEffect, Location location, int count, double speed, double offsetX,
                    double offsetY, double offsetZ) {
    particleEffect.display((float) offsetX, (float) offsetY, (float) offsetZ, (float) speed, count, location,
                           new ArrayList<>(Bukkit.getOnlinePlayers()));
  }

  @Override
  public void playerSpawn(Player player, ParticleEffect particleEffect, Location location, int count,
                          double speed, double offsetX, double offsetY, double offsetZ) {
    particleEffect
      .display((float) offsetX, (float) offsetY, (float) offsetZ, (float) speed, count, location, player);
  }
  ...
  @Override
  public void playerSpawn(Player player, ParticleEffect particleEffect, Location location,
                          ParticleColor color) {
    playerSpawn(player, particleEffect, location, 0, 1, color.getOffsetX(), color.getOffsetY(),
                color.getOffsetZ());
  }
