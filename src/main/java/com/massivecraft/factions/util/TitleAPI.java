...
public class TitleAPI {
    ...
    try {
      ...
      try {
        ...
        FactionsPlugin.getInstance().getLogger()
          .info("Didn't find API support for sending titles, using reflection instead.");
      } catch (Exception ex) {
        bailOut = true;
        FactionsPlugin.getInstance().getLogger().log(Level.SEVERE,
                                                     "Didn't find API support for sending titles, and " +
                                                     "failed to use reflection. Title support disabled.",
                                                     ex);
      }
    }
  }
   ...
   */
  public void sendTitle(Player player, String title, String subtitle, int fadeInTime, int showTime,
                        int fadeOutTime) {
    if (bailOut) {
      return;
    }
    ...
    try {
      ...
      Object titlePacket =
        titleConstructor.newInstance(fieldTitle.get(null), chatTitle, fadeInTime, showTime, fadeOutTime);
      Object subTitlePacket = titleConstructor
        .newInstance(fieldSubTitle.get(null), chatsubTitle, fadeInTime, showTime, fadeOutTime);
  ...
  private void sendPacket(Player player, Object packet) {
    try {
      ...
      FactionsPlugin.getInstance().getLogger()
        .log(Level.SEVERE, "Failed to send title packet via reflection", e);
    }
  }
  ...
  private Class<?> getNMSClass(String name) throws ClassNotFoundException {
    String versionName = "net.minecraft.server." + Bukkit.getServer().getClass().getPackage().getName()
      .split("\\.")[3] + "." + name;
