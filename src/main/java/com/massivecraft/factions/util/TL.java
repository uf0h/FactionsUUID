...
/**
 * An enum for requesting strings from the language file. The contents of this enum file may be subject to
 * frequent
 * changes.
 */
public enum TL {
  ...
  COMMAND_BOOM_PEACEFULONLY(
    "&cThis command is only usable by factions which are specifically designated as peaceful."),
  ...
  COMMAND_BYPASS_ENABLE(
    "&eYou have enabled admin bypass mode. You will be able to build or destroy anywhere."),
  ...
  COMMAND_CLAIMLINE_NOTVALID(
    "%s&c is not a cardinal direction. You may use &dnorth&c, &deast&c, &dsouth &cor &dwest&c."),
  ...
  COMMAND_CREATE_ERROR(
    "&cThere was an internal error while trying to create your faction. Please try again."),
  ...
  COMMAND_HOME_TELEPORTDISABLED(
    "&cSorry, the ability to teleport to Faction homes is disabled on this server."),
  COMMAND_HOME_NOHOME("&cYour faction does not have a home. "),
  COMMAND_HOME_INENEMY(
    "&cYou cannot teleport to your faction home while in the territory of an enemy faction."),
  COMMAND_HOME_WRONGWORLD("&cYou cannot teleport to your faction home while in a different world."),
  COMMAND_HOME_ENEMYNEAR(
    "&cYou cannot teleport to your faction home while an enemy is within %s blocks of you."),
  ...
  COMMAND_JOIN_ATLIMIT(
    " &c!&f The faction %1$s is at the limit of %2$d members, so %3$s cannot currently join."),
  ...
  COMMAND_OWNER_LIMIT(
    "&cSorry, but you have reached the server's &dlimit of %1$d &cowned areas per faction."),
  ...
  COMMAND_POWERBOOST_HELP_1(
    "&cYou must specify \"p\" or \"player\" to target a player or \"f\" or \"faction\" to target a faction."),
  COMMAND_POWERBOOST_HELP_2("&cex. /f powerboost p SomePlayer 0.5  -or-  /f powerboost f SomeFaction -5"),
  COMMAND_POWERBOOST_INVALIDNUM(
    "&cYou must specify a valid numeric value for the power bonus/penalty amount."),
  ...
  CLAIM_CONTIGIOUS(
    "&cYou can only claim additional land which is connected to your first claim or controlled by another " +
    "faction!"),
  CLAIM_FACTIONCONTIGUOUS("&cYou can only claim additional land which is connected to your first claim!"),
  CLAIM_PEACEFUL(
    "%s&e owns this land. Your faction is peaceful, so you cannot claim land from other factions."),
  ...
  PLAYER_POWER_LOSS_WARZONE(
    "&cThe world you are in has power loss normally disabled, but you still lost power since you were in a " +
  ...
  public String toString() {
    return this == TITLE ? ChatColor.translateAlternateColorCodes('&', LANG.getString(this.path, def)) + " " :
           ChatColor.translateAlternateColorCodes('&', LANG.getString(this.path, def));
  }
}
