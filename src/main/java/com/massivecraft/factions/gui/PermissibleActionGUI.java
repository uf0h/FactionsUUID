...
public class PermissibleActionGUI extends GUI<PermissibleAction> implements GUI.Backable {
  ...
  private static final SimpleItem backItem =
      SimpleItem.builder().setMaterial(Material.ARROW).setName(TL.GUI_BUTTON_BACK.toString()).build();
  ...
  static {
    base = SimpleItem.builder().setLore(FactionsPlugin.getInstance().conf().commands().perms().getGuiLore())
  ...
  protected String getName() {
    String bit = FactionsPlugin.getInstance().conf().factions().other().isSeparateOfflinePerms() ?
                 TL.GUI_PERMS_ACTION_ONLINEOFFLINEBIT
  ...
  protected String parse(String toParse, PermissibleAction action) {
    String actionName =
        action.getShortDescription().substring(0, 1).toUpperCase() + action.getShortDescription()
            .substring(1);
    ...
    toParse = toParse.replace("{action-access-color}",
        access ? ChatColor.GREEN.toString() : ChatColor.DARK_RED.toString());
  ...
  public void click(int slot, ClickType clickType) {
    if (FactionsPlugin.getInstance().conf().factions().other()
        .isSeparateOfflinePerms() && permissible instanceof Relation && slot == this.back + 4) {
  ...
  protected void onClick(PermissibleAction action, ClickType click) {
    ...
    if (user.getFaction().setPermission(online, permissible, action, access)) {
      ...
      buildItem(action);
      user.msg(TL.COMMAND_PERM_SET, action.getShortDescription(), access ? allowLower : denyLower,
          permissible.name());
      FactionsPlugin.getInstance().log(TL.COMMAND_PERM_SET
          .format(action.getShortDescription(), access ? "Allow" : "Deny",
              permissible.name()) + " for faction " + user.getTag());
    } else {
  ...
  protected Map<Integer, SimpleItem> createDummyItems() {
    ...
    if (FactionsPlugin.getInstance().conf().factions().other()
        .isSeparateOfflinePerms() && permissible instanceof Relation) {
      map.put(this.back + 4, PermissibleRelationGUI.offlineSwitch);
    }
    return map;
  }

  // For dummy items only parseDefault is called, but we want to provide the relation placeholders, so:
  // Override
  @Override
  protected String parseDefault(String string) {
    String permissibleName =
        permissible.toString().substring(0, 1).toUpperCase() + permissible.toString().substring(1);
