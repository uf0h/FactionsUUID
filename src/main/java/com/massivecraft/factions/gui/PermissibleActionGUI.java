...
public class PermissibleActionGUI extends GUI<PermissibleAction> implements GUI.Backable {
  ...
  protected String parse(String toParse, PermissibleAction action) {
    String actionName =
      action.getShortDescription().substring(0, 1).toUpperCase() + action.getShortDescription().substring(1);
    ...
    toParse = toParse
      .replace("{action-access-color}", access ? ChatColor.GREEN.toString() : ChatColor.DARK_RED.toString());
