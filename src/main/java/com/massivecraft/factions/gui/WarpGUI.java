...
public class WarpGUI extends GUI<Integer> {
  ...
  private WarpGUI(FPlayer user, int page, Faction faction) {
    ...
    this.page = page;
    name = page == -1 ? TL.GUI_WARPS_ONE_PAGE.format(faction.getTag()) :
           TL.GUI_WARPS_PAGE.format(faction.getTag(), page + 1);
    build();
  }
  ...
  private void doWarmup(final String warp) {
    WarmUpUtil.process(user, WarmUpUtil.Warmup.WARP, TL.WARMUPS_NOTIFY_TELEPORT, warp, () -> {
      ...
      if (player != null) {
        ...
        FactionsPlugin.getInstance().teleport(player, faction.getWarp(warp).getLocation())
          .thenAccept(success -> {
            if (success) {
  ...
  private boolean transact() {
    ...
    if (FactionsPlugin.getInstance().conf().economy().isBankEnabled() && FactionsPlugin.getInstance().conf()
      .economy().isBankFactionPaysCosts() && user.hasFaction() && user.getFaction()
          .hasAccess(user, PermissibleAction.ECONOMY)) {
      return Econ.modifyMoney(user.getFaction(), -cost, TL.COMMAND_FWARP_TOWARP.toString(),
                              TL.COMMAND_FWARP_FORWARPING.toString());
    } else {
      return Econ
        .modifyMoney(user, -cost, TL.COMMAND_FWARP_TOWARP.toString(), TL.COMMAND_FWARP_FORWARPING.toString());
    }
  }
