...
public abstract class FRelationCommand extends FCommand {
  ...
  public void perform(CommandContext context) {
    ...
    FactionRelationWishEvent wishEvent =
      new FactionRelationWishEvent(context.fPlayer, context.faction, them, oldRelation, targetRelation);
    ...
    if (!context.payForCommand(targetRelation.getRelationCost(), TL.COMMAND_RELATIONS_TOMARRY,
                               TL.COMMAND_RELATIONS_FORMARRY)) {
      return;
    }
    ...
    if (targetRelation.value == currentRelation.value) {
      ...
      FactionRelationEvent relationEvent =
        new FactionRelationEvent(context.faction, them, oldRelation, currentRelation);
      Bukkit.getServer().getPluginManager().callEvent(relationEvent);

      them.msg(TL.COMMAND_RELATIONS_MUTUAL, currentRelationColor + targetRelation.getTranslation(),
               currentRelationColor + context.faction.getTag());
      context.faction.msg(TL.COMMAND_RELATIONS_MUTUAL, currentRelationColor + targetRelation.getTranslation(),
                          currentRelationColor + them.getTag());
    } else {
      // inform the other faction of your request
      them.msg(TL.COMMAND_RELATIONS_PROPOSAL_1, currentRelationColor + context.faction.getTag(),
               targetRelation.getColor() + targetRelation.getTranslation());
      them.msg(TL.COMMAND_RELATIONS_PROPOSAL_2, FactionsPlugin.getInstance().conf().getCommandBase().get(0),
               targetRelation, context.faction.getTag());
      context.faction.msg(TL.COMMAND_RELATIONS_PROPOSAL_SENT, currentRelationColor + them.getTag(),
                          "" + targetRelation.getColor() + targetRelation);
    }
