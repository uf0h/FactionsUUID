...
public class CmdHelp extends FCommand {
  ...
  public void updateHelp(CommandContext context) {
    ...
    if (Econ.isSetup() && FactionsPlugin.getInstance().conf().economy().isEnabled() && FactionsPlugin
      .getInstance().conf().economy().isBankEnabled()) {
      pageLines = new ArrayList<>();
      pageLines.add("");
    ...
    //TODO:TL
    pageLines.add(plugin.txt().parse(
      "<i>Note: " + FCmdRoot.getInstance().cmdUnclaim.getUsageTemplate(context, false) + FactionsPlugin
        .getInstance().txt().parse("<i>") + " works on safe/war zones as well."));
