...
public class CmdWarp extends FCommand {
  ...
  public void perform(CommandContext context) {
    ...
    if (context.args.size() == 0) {
      ...
      if (context.faction.isWarp(context.argAsString(0))) {
        ...
        if (!context.fPlayer.isAdminBypassing() && context.faction
          .hasWarpPassword(warpName) && !context.faction.isWarpPassword(warpName, passwordAttempt)) {
          context.fPlayer.msg(TL.COMMAND_FWARP_INVALID_PASSWORD);
          return;
        }
        FPlayerTeleportEvent tpEvent = new FPlayerTeleportEvent(context.fPlayer,
                                                                context.fPlayer.getFaction().getWarp(warpName)
                                                                  .getLocation(),
                                                                FPlayerTeleportEvent.PlayerTeleportReason.WARP);
        ...
        context.doWarmUp(WarmUpUtil.Warmup.WARP, TL.WARMUPS_NOTIFY_TELEPORT, warpName, () -> {
          ...
          if (player != null) {
            FactionsPlugin.getInstance()
              .teleport(player, fPlayer.getFaction().getWarp(warpName).getLocation()).thenAccept(success -> {
              if (success) {
  ...
  private boolean transact(FPlayer player, CommandContext context) {
    return player.isAdminBypassing() || context
      .payForCommand(FactionsPlugin.getInstance().conf().economy().getCostWarp(),
                     TL.COMMAND_FWARP_TOWARP.toString(), TL.COMMAND_FWARP_FORWARPING.toString());
  }
