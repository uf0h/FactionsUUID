...
public class CmdPermanent extends FCommand {
  ...
  public void perform(CommandContext context) {
    ...
    FactionsPlugin.getInstance().log((context.fPlayer == null ? "A server admin" :
                                      context.fPlayer.getName()) + " " + change + " the faction \"" + faction
        .getTag() + "\".");

    // Inform all players
    for (FPlayer fplayer : FPlayers.getInstance().getOnlinePlayers()) {
      String blame = (context.fPlayer == null ? TL.GENERIC_SERVERADMIN.toString() :
