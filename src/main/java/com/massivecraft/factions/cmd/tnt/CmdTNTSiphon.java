...
public class CmdTNTSiphon extends FCommand {
  ...
  public CmdTNTSiphon() {
    ...
    this.requirements =
        new CommandRequirements.Builder(Permission.TNT_SIPHON).withAction(PermissibleAction.TNTDEPOSIT)
            .memberOnly().build();
  }
  ...
  public void perform(CommandContext context) {
    if (!context.faction
        .equals(Board.getInstance().getFactionAt(new FLocation(context.player.getLocation())))) {
    ...
    if (FactionsPlugin.getInstance().conf().commands().tnt()
        .isAboveMaxStorage(context.faction.getTNTBank() + 1)) {
      context.msg(TL.COMMAND_TNT_SIPHON_FAIL_FULL);
      return;
    }

    if (radius > FactionsPlugin.getInstance().conf().commands().tnt().getMaxRadius()) {
      context.msg(TL.COMMAND_TNT_SIPHON_FAIL_MAXRADIUS, radius,
          FactionsPlugin.getInstance().conf().commands().tnt().getMaxRadius());
      return;
    }

    List<Dispenser> list =
        CmdTNTFill.getDispensers(context.player.getLocation(), radius, context.faction.getId());
    ...
    for (Dispenser dispenser : list) {
      if (remaining > (64 * 9)) {
        ...
        remaining = CmdTNTFill
            .getCount(dispenser.getInventory().removeItem(CmdTNTFill.getStacks(remaining)).values());
      }
      if (remaining == 0) {
        break;
      }
