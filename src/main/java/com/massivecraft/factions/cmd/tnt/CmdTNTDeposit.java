...
public class CmdTNTDeposit extends FCommand {
  ...
  public CmdTNTDeposit() {
    ...
    this.requirements =
        new CommandRequirements.Builder(Permission.TNT_DEPOSIT).withAction(PermissibleAction.TNTDEPOSIT)
            .memberOnly().build();
  }
  ...
  public void perform(CommandContext context) {
    ...
    if (FactionsPlugin.getInstance().conf().commands().tnt()
        .isAboveMaxStorage(context.faction.getTNTBank() + amount)) {
      if (FactionsPlugin.getInstance().conf().commands().tnt().getMaxStorage() == context.faction
          .getTNTBank()) {
        context.msg(TL.COMMAND_TNT_DEPOSIT_FAIL_FULL,
            FactionsPlugin.getInstance().conf().commands().tnt().getMaxStorage());
        return;
      }
      amount =
          FactionsPlugin.getInstance().conf().commands().tnt().getMaxStorage() - context.faction.getTNTBank();
    }
    int current = amount;
