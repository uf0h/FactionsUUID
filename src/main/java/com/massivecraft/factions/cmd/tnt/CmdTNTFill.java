...
public class CmdTNTFill extends FCommand {
  ...
  public CmdTNTFill() {
    ...
    this.requirements =
      new CommandRequirements.Builder(Permission.TNT_FILL).withAction(PermissibleAction.TNTWITHDRAW)
        .memberOnly().build();
  }
  ...
  public void perform(CommandContext context) {
    ...
    if (radius > FactionsPlugin.getInstance().conf().commands().tnt().getMaxRadius()) {
      context.msg(TL.COMMAND_TNT_FILL_FAIL_MAXRADIUS, radius,
    ...
    context
      .msg(TL.COMMAND_TNT_FILL_MESSAGE, amount - remaining, dispenserCount, context.faction.getTNTBank());
  }
      ...
      for (int y = location.getBlockY() - radius; y <= location.getBlockY() + radius; y++) {
        for (int z = location.getBlockZ() - radius; z <= location.getBlockZ() + radius; z++) {
          ...
          if (block.getType() == Material.DISPENSER) {
            list.add(Pair.of((Dispenser) block.getState(), Math.sqrt(
              ((location.getBlockX() - x) ^ 2) + ((location.getBlockY() - y) ^ 2) + ((location
                                                                                        .getBlockZ() - z) ^ 2))));
          }
        }
      }
