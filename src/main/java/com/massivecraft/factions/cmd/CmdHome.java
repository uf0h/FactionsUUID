...
public class CmdHome extends FCommand {
  ...
  public void perform(final CommandContext context) {
    ...
    if (!FactionsPlugin.getInstance().conf().factions().homes()
        .isTeleportAllowedFromEnemyTerritory() && context.fPlayer.isInEnemyTerritory()) {
      context.fPlayer.msg(TL.COMMAND_HOME_INENEMY);
      return;
    }

    if (!FactionsPlugin.getInstance().conf().factions().homes()
        .isTeleportAllowedFromDifferentWorld() && context.player.getWorld().getUID() != targetFaction
        .getHome().getWorld().getUID()) {
    ...
    if (FactionsPlugin.getInstance().conf().factions().homes().getTeleportAllowedEnemyDistance() > 0 &&
        !faction.isSafeZone() &&
        (!context.fPlayer.isInOwnTerritory() || !FactionsPlugin.getInstance().conf().factions().homes()
            .isTeleportIgnoreEnemiesIfInOwnTerritory())) {
      ...
      for (Player p : context.player.getServer().getOnlinePlayers()) {
        ...
        context.fPlayer.msg(TL.COMMAND_HOME_ENEMYNEAR, String.valueOf(
            FactionsPlugin.getInstance().conf().factions().homes().getTeleportAllowedEnemyDistance()));
        return;
      }
    }

    Location destination = targetFaction.getHome();
    FPlayerTeleportEvent tpEvent = new FPlayerTeleportEvent(context.fPlayer, destination,
        FPlayerTeleportEvent.PlayerTeleportReason.HOME);
    ...
    if (!context.payForCommand(FactionsPlugin.getInstance().conf().economy().getCostHome(),
        TL.COMMAND_HOME_TOTELEPORT.toString(), TL.COMMAND_HOME_FORTELEPORT.toString())) {
      return;
    }

    // if Essentials teleport handling is enabled and available, pass the teleport off to it (for delay and
    // cooldown)
    ...
    context.doWarmUp(WarmUpUtil.Warmup.HOME, TL.WARMUPS_NOTIFY_TELEPORT, "Home", () -> {
      ...
      if (FactionsPlugin.getInstance().conf().factions().homes().isTeleportCommandSmokeEffectEnabled()) {
        ...
        SmokeUtil.spawnCloudRandom(smokeLocations,
            FactionsPlugin.getInstance().conf().factions().homes().getTeleportCommandSmokeEffectThickness());
      }
