...
public class CmdMoneyTransferFf extends MoneyCommand {
  ...
  public void perform(CommandContext context) {
    ...
    if (success && FactionsPlugin.getInstance().conf().logging().isMoneyTransactions()) {
      ...
      FactionsPlugin.getInstance().log(ChatColor.stripColor(FactionsPlugin.getInstance().txt().parse(
        TL.COMMAND_MONEYTRANSFERFF_TRANSFER.toString(), name, Econ.moneyString(amount), from.describeTo(null),
        to.describeTo(null))));
    }
  }
