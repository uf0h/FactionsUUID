...
public class CmdClaimFill extends FCommand {
  ...
  public CmdClaimFill() {
    ...
    // Args
    this.optionalArgs.put("limit", String
      .valueOf(FactionsPlugin.getInstance().conf().factions().claims().getFillClaimMaxClaims()));
  ...
  public void perform(CommandContext context) {
    // Args
    final int limit =
      context.argAsInt(0, FactionsPlugin.getInstance().conf().factions().claims().getFillClaimMaxClaims());

    if (limit > FactionsPlugin.getInstance().conf().factions().claims().getFillClaimMaxClaims()) {
      context.msg(TL.COMMAND_CLAIMFILL_ABOVEMAX,
                  FactionsPlugin.getInstance().conf().factions().claims().getFillClaimMaxClaims());
      return;
    }
    ...
    if (!context.fPlayer.isAdminBypassing() && !forFaction
      .hasAccess(context.fPlayer, PermissibleAction.TERRITORY)) {
    ...
    while (!queue.isEmpty() && toClaim.size() <= limit) {
      ...
      if (Math.abs(currentHead.getX() - startX) > distance || Math.abs(
        currentHead.getZ() - startZ) > distance) {
    ...
    if (toClaim.size() > this.plugin.getLandRaidControl().getPossibleClaimCount(forFaction)) {
      context
        .msg(TL.COMMAND_CLAIMFILL_NOTENOUGHLANDLEFT, forFaction.describeTo(context.fPlayer), toClaim.size());
      return;
    }
