...
public class CmdClaimFill extends FCommand {
  ...
  public CmdClaimFill() {
    ...
    // Args
    this.optionalArgs.put("limit",
        String.valueOf(FactionsPlugin.getInstance().conf().factions().claims().getFillClaimMaxClaims()));
  ...
  public void perform(CommandContext context) {
    ...
    while (!queue.isEmpty() && toClaim.size() <= limit) {
      ...
      if (Math.abs(currentHead.getX() - startX) > distance || Math
          .abs(currentHead.getZ() - startZ) > distance) {
    ...
    if (toClaim.size() > this.plugin.getLandRaidControl().getPossibleClaimCount(forFaction)) {
      context.msg(TL.COMMAND_CLAIMFILL_NOTENOUGHLANDLEFT, forFaction.describeTo(context.fPlayer),
          toClaim.size());
      return;
    }
