...
public class CmdVault extends FCommand {
  ...
  public void perform(CommandContext context) {
    ...
    String vaultName = String
        .format(FactionsPlugin.getInstance().conf().playerVaults().getVaultPrefix(), context.faction.getId());
    ...
    if (number < 1) {
      ...
      if (file == null) {
        ...
        context.sender.sendMessage(
            Lang.TITLE.toString() + Lang.EXISTING_VAULTS.toString().replaceAll("%p", context.fPlayer.getTag())
                .replaceAll("%v", sb.toString().trim()));
      }
      return;
    } // end listing vaults.
    ...
    if (VaultOperations.openOtherVault(player, vaultName, String.valueOf(number))) {
      ...
      PlayerVaults.getInstance().getInVault()
          .put(player.getUniqueId().toString(), new VaultViewInfo(vaultName, number));
    }
  }
  ...
  public TL getUsageTranslation() {
    ...
    /*protected class VaultBrigadier implements BrigadierProvider {
        ...
        public ArgumentBuilder<Object, ?> get(ArgumentBuilder<Object, ?> parent) {
            return parent.then(RequiredArgumentBuilder.argument("number", IntegerArgumentType.integer(0,
            99)));
        }
    }*/
