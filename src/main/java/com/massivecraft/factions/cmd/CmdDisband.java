...
public class CmdDisband extends FCommand {
  ...
  public void perform(CommandContext context) {
    ...
    for (FPlayer fplayer : FPlayers.getInstance().getOnlinePlayers()) {
      String who =
        context.player == null ? TL.GENERIC_SERVERADMIN.toString() : context.fPlayer.describeTo(fplayer);
    ...
    if (FactionsPlugin.getInstance().conf().logging().isFactionDisband()) {
      ...
      FactionsPlugin.getInstance().log(
        "The faction " + faction.getTag() + " (" + faction.getId() + ") was disbanded by " + (
          context.player == null ? "console command" : context.fPlayer.getName()) + ".");
    }
    ...
    if (Econ.shouldBeUsed() && context.player != null) {
      ...
      if (amount > 0.0) {
        ...
        //TODO: Format this correctly and translate
        FactionsPlugin.getInstance().log(context.fPlayer
                                           .getName() + " has been given bank holdings of " + amountString + " from disbanding " + faction
                                           .getTag() + ".");
      }
    }
