...
public class CmdDescription extends FCommand {
  ...
  public void perform(CommandContext context) {
    ...
    if (!context.payForCommand(FactionsPlugin.getInstance().conf().economy().getCostDesc(),
        TL.COMMAND_DESCRIPTION_TOCHANGE, TL.COMMAND_DESCRIPTION_FORCHANGE)) {
      return;
    }

    // since "&" color tags seem to work even through plain old FPlayer.sendMessage() for some reason, we
    // need to break those up
    // And replace all the % because it messes with string formatting and this is easy way around that.
    context.faction.setDescription(
        TextUtil.implode(context.args, " ").replaceAll("%", "").replaceAll("(&([a-f0-9klmnor]))", "& $2"));
    ...
    for (FPlayer fplayer : FPlayers.getInstance().getOnlinePlayers()) {
      ...
      fplayer.sendMessage(context.faction
          .getDescription());  // players can inject "&" or "`" or "<i>" or whatever in their description;
      // &k is particularly interesting looking
    }
  }
