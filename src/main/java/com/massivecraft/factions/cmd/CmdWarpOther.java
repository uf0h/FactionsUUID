  ...
  public void perform(CommandContext context) {
    ...
    if (context.args.size() == 1) {
      ...
      if (faction.isWarp(warpName)) {
        ...
        if (!context.fPlayer.isAdminBypassing() && faction.hasWarpPassword(warpName) && !faction
          .isWarpPassword(warpName, passwordAttempt)) {
          context.fPlayer.msg(TL.COMMAND_FWARP_INVALID_PASSWORD);
          return;
        }
        FPlayerTeleportEvent tpEvent =
          new FPlayerTeleportEvent(context.fPlayer, faction.getWarp(warpName).getLocation(),
        ...
        context.doWarmUp(WarmUpUtil.Warmup.WARP, TL.WARMUPS_NOTIFY_TELEPORT, warpName, () -> {
          ...
          if (player != null) {
            FactionsPlugin.getInstance().teleport(player, faction.getWarp(warpName).getLocation())
              .thenAccept(success -> {
                if (success) {
