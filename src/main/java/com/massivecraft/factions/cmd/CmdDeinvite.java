  ...
  public void perform(CommandContext context) {
    ...
    if (you == null) {
      ...
      for (UUID id : context.faction.getInvites()) {
        ...
        msg.then(name + " ").color(ChatColor.WHITE).tooltip(TL.COMMAND_DEINVITE_CLICKTODEINVITE.format(name))
          .command("/" + FactionsPlugin.getInstance().conf().getCommandBase().get(0) + " deinvite " + name);
      }
    ...
    context.faction.msg(TL.COMMAND_DEINVITE_REVOKES, context.fPlayer.describeTo(context.faction),
                        you.describeTo(context.faction));
  }
