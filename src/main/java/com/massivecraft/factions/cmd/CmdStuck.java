...
public class CmdStuck extends FCommand {
  ...
  public void perform(final CommandContext context) {
    ...
    if (FactionsPlugin.getInstance().getStuckMap().containsKey(player.getUniqueId())) {
      long wait =
        FactionsPlugin.getInstance().getTimers().get(player.getUniqueId()) - System.currentTimeMillis();
      ...
      FPlayerTeleportEvent tpEvent =
        new FPlayerTeleportEvent(context.fPlayer, null, FPlayerTeleportEvent.PlayerTeleportReason.STUCK);
      ...
      if (!context.payForCommand(FactionsPlugin.getInstance().conf().economy().getCostStuck(),
      ...
      final int id = new BukkitRunnable() {
        ...
        public void run() {
          ...
          if (world.getUID() != player.getWorld().getUID() || sentAt
      ...
      FactionsPlugin.getInstance().getTimers()
        .put(player.getUniqueId(), System.currentTimeMillis() + (delay * 1000));
      long wait =
        FactionsPlugin.getInstance().getTimers().get(player.getUniqueId()) - System.currentTimeMillis();
      String time = DurationFormatUtils.formatDuration(wait, TL.COMMAND_STUCK_TIMEFORMAT.toString(), true);
