...
public class CommandContext {
  ...
  public Boolean strAsBool(String str) {
    str = str.toLowerCase();
    return str.startsWith("y") || str.startsWith("t") || str.startsWith("on") || str.startsWith("+") || str
      .startsWith("1");
  }
  ...
  */
  public boolean canIAdministerYou(FPlayer i, FPlayer you) {
    if (!i.getFaction().equals(you.getFaction())) {
      i.sendMessage(FactionsPlugin.getInstance().txt()
                      .parse("%s <b>is not in the same faction as you.", you.describeTo(i, true)));
      return false;
    }
    ...
    i.sendMessage(
      FactionsPlugin.getInstance().txt().parse("%s <b>has a higher rank than you.", you.describeTo(i, true)));

    return false;
  }

  // if economy is enabled and they're not on the bypass list, make 'em pay; returns true unless person
  ...
  public boolean payForCommand(double cost, String toDoThis, String forDoingThis) {
    ...
    if (FactionsPlugin.getInstance().conf().economy().isBankEnabled() && FactionsPlugin.getInstance().conf()
      .economy().isBankFactionPaysCosts() && fPlayer.hasFaction() && fPlayer.getFaction()
          .hasAccess(fPlayer, PermissibleAction.ECONOMY)) {
  ...
  public boolean canAffordCommand(double cost, String toDoThis) {
    ...
    if (FactionsPlugin.getInstance().conf().economy().isBankEnabled() && FactionsPlugin.getInstance().conf()
      .economy().isBankFactionPaysCosts() && fPlayer.hasFaction()) {
  ...
  public void doWarmUp(WarmUpUtil.Warmup warmup, TL translationKey, String action, Runnable runnable,
                       long delay) {
    this.doWarmUp(fPlayer, warmup, translationKey, action, runnable, delay);
  }

  public void doWarmUp(FPlayer player, WarmUpUtil.Warmup warmup, TL translationKey, String action,
                       Runnable runnable, long delay) {
