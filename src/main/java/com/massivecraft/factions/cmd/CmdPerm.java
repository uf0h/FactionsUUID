...
public class CmdPerm extends FCommand {
  ...
  public void perform(CommandContext context) {
    if (context.args.size() == 0) {
      ...
      PermissibleActionGUI ui =
          new PermissibleActionGUI(true, context.fPlayer, getPermissible(context.argAsString(0)));
      ui.open();
      return;
    }
    ...
    boolean online = true;
    if (FactionsPlugin.getInstance().conf().factions().other().isSeparateOfflinePerms() && context.args
        .size() == 4 && "offline".equalsIgnoreCase(context.argAsString(3))) {
      online = false;
    }
    ...
    FactionsPlugin.getInstance().log(String
        .format(TL.COMMAND_PERM_SET.toString(), context.argAsString(1), access,
            context.argAsString(0)) + " for faction " + context.fPlayer.getTag());
  }
