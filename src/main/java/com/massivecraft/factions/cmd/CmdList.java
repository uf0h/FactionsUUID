...
public class CmdList extends FCommand {
  ...
  public void perform(CommandContext context) {
    ...
    if (!context
      .payForCommand(FactionsPlugin.getInstance().conf().economy().getCostList(), "to list the factions",
    ...
    if (!header.isEmpty()) {
      header = header.replace("{pagenumber}", String.valueOf(pagenumber))
    ...
    for (Faction faction : factionList.subList(start, end)) {
      if (faction.isWilderness()) {
        lines.add(
          plugin.txt().parse(Tag.parsePlain(faction, plugin.conf().commands().list().getFactionlessEntry())));
        continue;
      }
      lines.add(plugin.txt().parse(
        Tag.parsePlain(faction, context.fPlayer, plugin.conf().commands().list().getEntry())));
    }

    if (!footer.isEmpty()) {
      footer = footer.replace("{pagenumber}", String.valueOf(pagenumber))
        .replace("{pagecount}", String.valueOf(pagecount));
