  ...
  public void perform(CommandContext context) {
    ...
    for (Faction faction : factionList.subList(start, end)) {
      if (faction.isWilderness()) {
        lines.add(plugin.txt()
            .parse(Tag.parsePlain(faction, plugin.conf().commands().list().getFactionlessEntry())));
        continue;
      }
      lines.add(plugin.txt()
          .parse(Tag.parsePlain(faction, context.fPlayer, plugin.conf().commands().list().getEntry())));
    }
