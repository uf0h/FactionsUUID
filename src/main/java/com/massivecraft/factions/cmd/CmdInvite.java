...
public class CmdInvite extends FCommand {
  ...
  public void perform(CommandContext context) {
    ...
    if (target.getFaction() == context.faction) {
      ...
      context
        .msg(TL.GENERIC_YOUMAYWANT.toString() + FCmdRoot.getInstance().cmdKick.getUsageTemplate(context));
      return;
    }

    // if economy is enabled, they're not on the bypass list, and this command has a cost set, make 'em pay
    if (!context.payForCommand(FactionsPlugin.getInstance().conf().economy().getCostInvite(),
                               TL.COMMAND_INVITE_TOINVITE.toString(),
                               TL.COMMAND_INVITE_FORINVITE.toString())) {
      return;
    }
    ...
    FancyMessage message = new FancyMessage(context.fPlayer.describeTo(target, true))
      ...
      .command("/" + FactionsPlugin.getInstance().conf().getCommandBase().get(0) + " join " + context.faction
        .getTag())
    ...
    //you.msg("%s<i> invited you to %s",context.fPlayer.describeTo(you, true), context.faction.describeTo
    // (you));
    context.faction.msg(TL.COMMAND_INVITE_INVITED, context.fPlayer.describeTo(context.faction, true),
                        target.describeTo(context.faction));
  }
