...
public class FCmdRoot extends FCommand implements CommandExecutor {
  ...
  public FCmdRoot() {
    ...
    this.helpLong
        .add(FactionsPlugin.getInstance().txt().parseTags("<i>This command contains all faction stuff."));
    ...
    if (FactionsPlugin.getInstance().getLandRaidControl() instanceof PowerControl) {
      FactionsPlugin.getInstance().getLogger()
          .info("Using POWER for land/raid control. Enabling power commands.");
      ...
      FactionsPlugin.getInstance().getLogger()
          .info("Using DTR for land/raid control. Enabling DTR commands.");
    ...
    if (FactionsPlugin.getInstance().conf().commands().fly().isEnable()) {
      ...
      FactionsPlugin.getInstance().getLogger()
          .info("Faction flight set to false in main.conf. Not enabling /f fly command.");
    }
  ...
  public void done() {
    if (Bukkit.getServer().getPluginManager().isPluginEnabled("FactionsTop")) {
      FactionsPlugin.getInstance().getLogger()
          .info("Found FactionsTop plugin. Disabling our own /f top command.");
    } else {
      this.addSubCommand(this.cmdTop);
    }
    if (Bukkit.getServer().getPluginManager().isPluginEnabled("PlayerVaults")) {
      FactionsPlugin.getInstance().getLogger()
          .info("Found PlayerVaults hook, adding /f vault and /f setmaxvault commands.");
