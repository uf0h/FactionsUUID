...
public class CmdNear extends FCommand {
  ...
  public void perform(CommandContext context) {
    ...
    for (FPlayer player : onlineMembers) {
      ...
      if (cur.getWorld().getUID().equals(loc.getWorld().getUID()) && cur.distanceSquared(
        loc) <= radiusSquared) {
    ...
    context.msg(
      TL.COMMAND_NEAR_PLAYERLIST.toString().replace("{players-nearby}", playerMessageBuilder.toString()));
  }
