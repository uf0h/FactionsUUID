...
public class CmdShow extends FCommand {
  ...
  public void perform(CommandContext context) {
    ...
    if (!context
      .payForCommand(FactionsPlugin.getInstance().conf().economy().getCostShow(), TL.COMMAND_SHOW_TOSHOW,
                     TL.COMMAND_SHOW_FORSHOW)) {
      return;
    }
    ...
    if (context.fPlayer != null && this.groupPresent()) {
      new GroupGetter(messageList, context.fPlayer, faction)
  ...
  private void sendMessages(List<String> messageList, CommandSender recipient, Faction faction,
                            FPlayer player) {
    this.sendMessages(messageList, recipient, faction, player, null);
  }

  private void sendMessages(List<String> messageList, CommandSender recipient, Faction faction,
                            FPlayer player, Map<UUID, String> groupMap) {
    FancyTag tag;
  ...
  private void onOffLineMessage(StringBuilder builder, CommandSender recipient, Faction faction,
                                boolean online) {
    boolean first = true;
  ...
  private void relationMessage(StringBuilder builder, CommandSender recipient, Faction faction,
                               Relation relation) {
    boolean first = true;
  ...
  private class GroupGetter extends BukkitRunnable {
    ...
    private GroupGetter(List<String> messageList, FPlayer sender, Faction faction) {
      ...
      this.players = faction.getFPlayers().stream().map(fp -> Bukkit.getOfflinePlayer(fp.getUniqueId()))
