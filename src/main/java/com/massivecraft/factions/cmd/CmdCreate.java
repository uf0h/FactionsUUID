...
public class CmdCreate extends FCommand {
  ...
  public void perform(CommandContext context) {
    ...
    // they can pay
    if (!context.canAffordCommand(FactionsPlugin.getInstance().conf().economy().getCostCreate(),
                                  TL.COMMAND_CREATE_TOCREATE.toString())) {
      return;
    }
    ...
    if (!context.payForCommand(FactionsPlugin.getInstance().conf().economy().getCostCreate(),
                               TL.COMMAND_CREATE_TOCREATE, TL.COMMAND_CREATE_FORCREATE)) {
      return;
    }
    ...
    FPlayerJoinEvent joinEvent =
      new FPlayerJoinEvent(FPlayers.getInstance().getByPlayer(context.player), faction,
                           FPlayerJoinEvent.PlayerJoinReason.CREATE);
    ...
    for (FPlayer follower : FPlayers.getInstance().getOnlinePlayers()) {
      follower
        .msg(TL.COMMAND_CREATE_CREATED, context.fPlayer.describeTo(follower, true), faction.getTag(follower));
    }
    ...
    if (FactionsPlugin.getInstance().conf().logging().isFactionCreate()) {
      FactionsPlugin.getInstance()
        .log(context.fPlayer.getName() + TL.COMMAND_CREATE_CREATEDLOG.toString() + tag);
    }
  }
