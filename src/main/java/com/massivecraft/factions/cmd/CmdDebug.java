...
public class CmdDebug extends FCommand {
  ...
  public void perform(CommandContext context) {
    ...
    mainInfo.append(Bukkit.getName()).append(" version: ").append(Bukkit.getServer().getVersion())
        .append('\n');
    mainInfo.append("Server ID: ").append(FactionsPlugin.getInstance().getServerUUID()).append('\n');
    mainInfo.append("Plugin version: ").append(FactionsPlugin.getInstance().getDescription().getVersion())
        .append('\n');
    ...
    for (Plugin plugin : Bukkit.getPluginManager().getPlugins()) {
      mainInfo.append(' ').append(plugin.getName()).append(" - ").append(plugin.getDescription().getVersion())
          .append('\n');
      ...
      permInfo.append('\n');
        ...
        /*new BukkitRunnable() {
            ...
            private void add(String name, String content) {
                builder.addFile(new PasteFile(i++ + name, new PasteContent(PasteContent.ContentType.TEXT,
                content)));
            }
            ...
            public void run() {
                try {
                    ...
                    add("server.properties", getFile(Paths.get("server.properties")).replaceAll("(?:
                    (?:server-ip=)|(?:server-port=)|(?:rcon\\.port=)|(?:rcon\\.password=)|(?:query.port=))
                    [^\n]*[\r\n]*", ""));
                    ...
                    new BukkitRunnable() {
                        ...
                        public void run() {
                            if (result.getPaste().isPresent()) {
                                String delKey = result.getPaste().get().getDeletionKey().orElse("No
                                deletion key");
                                context.msg(TL.COMMAND_DEBUG_COMPLETE, "https://paste.gg/anonymous/" +
                                result.getPaste().get().getId());
                                context.msg(TL.COMMAND_DEBUG_DELETIONKEY, delKey);
                                if (context.sender instanceof Player) {
                                    FactionsPlugin.getInstance().getLogger().info(TL.COMMAND_DEBUG_COMPLETE
                                    .format("https://paste.gg/anonymous/" + result.getPaste().get().getId()));
                                    FactionsPlugin.getInstance().getLogger().info(TL
                                    .COMMAND_DEBUG_DELETIONKEY.format(delKey));
                                }
                            } else {
                                context.msg(TL.COMMAND_DEBUG_FAIL);
                                FactionsPlugin.getInstance().getLogger().warning("Received: " + result
                                .getMessage());
                            }
                        }
                    }.runTask(FactionsPlugin.getInstance());
                } catch (Exception e) {
                    FactionsPlugin.getInstance().getLogger().log(Level.SEVERE, "Failed to execute debug
                    command", e);
                    new BukkitRunnable() {
                        @Override
                        public void run() {
