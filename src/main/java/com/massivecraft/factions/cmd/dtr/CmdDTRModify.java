...
public class CmdDTRModify extends FCommand {
  ...
  public void perform(CommandContext context) {
    ...
    target.setDTR(Math.max(Math.min(target.getDTR() + amount, dtr.getMaxDTR(target)),
        FactionsPlugin.getInstance().conf().factions().landRaidControl().dtr().getMinDTR()));
    context.msg(TL.COMMAND_DTR_MODIFY_DONE, target.describeTo(context.fPlayer, false),
        DTRControl.round(target.getDTR()));
  }
