...
public class CmdBan extends FCommand {
  ...
  public void perform(CommandContext context) {
    ...
    if (context.fPlayer == target) {
      ...
      return;
    } else if (target.getFaction() == context.faction && target.getRole().value >= context.fPlayer
      .getRole().value) {
    ...
    for (BanInfo banInfo : context.faction.getBannedPlayers()) {
      if (banInfo.getBanned().equals(target.getUniqueId())) {
    ...
    if (target.getFaction() == context.faction) {
      ...
      FPlayerLeaveEvent event =
        new FPlayerLeaveEvent(target, context.faction, FPlayerLeaveEvent.PlayerLeaveReason.BANNED);
      Bukkit.getServer().getPluginManager().callEvent(event);

      if (event.isCancelled()) {
        // if someone cancels a ban, we'll get people complaining here. So lets log it.
        FactionsPlugin.getInstance()
          .log(Level.WARNING, "Attempted to ban {0} but someone cancelled the kick event. This isn't good.",
               target.getName());
        return;
      }
