...
public class PowerControl implements LandRaidControl {
  ...
  public boolean isRaidable(Faction faction) {
    return FactionsPlugin.getInstance().conf().factions().landRaidControl().power().isRaidability() && faction
      .isNormal() && !faction.isPeaceful() && faction.getLandRounded() >= faction.getPowerRounded();
  }
  ...
  public boolean canJoinFaction(Faction faction, FPlayer player, CommandContext context) {
    if (!FactionsPlugin.getInstance().conf().factions().landRaidControl().power()
      .canLeaveWithNegativePower() && player.getPower() < 0) {
      if (context != null) {
  ...
  public boolean canKick(FPlayer toKick, CommandContext context) {
    if (!FactionsPlugin.getInstance().conf().factions().landRaidControl().power()
      .canLeaveWithNegativePower() && toKick.getPower() < 0) {
  ...
  public void onDeath(Player player) {
    ...
    if (faction.isWarZone()) {
      ...
      if (FactionsPlugin.getInstance().conf().factions().landRaidControl().power().getWorldsNoPowerLoss()
        .contains(player.getWorld().getName())) {
        powerLossEvent.setMessage(TL.PLAYER_POWER_LOSS_WARZONE.toString());
      }
    } else if (faction.isWilderness() && !FactionsPlugin.getInstance().conf().factions().landRaidControl()
      .power().isWildernessPowerLoss() && !FactionsPlugin.getInstance().conf().factions().protection()
      .getWorldsNoWildernessProtection().contains(player.getWorld().getName())) {
      powerLossEvent.setMessage(TL.PLAYER_POWER_NOLOSS_WILDERNESS.toString());
      powerLossEvent.setCancelled(true);
    } else if (FactionsPlugin.getInstance().conf().factions().landRaidControl().power().getWorldsNoPowerLoss()
      .contains(player.getWorld().getName())) {
      powerLossEvent.setMessage(TL.PLAYER_POWER_NOLOSS_WORLD.toString());
      powerLossEvent.setCancelled(true);
    } else if (FactionsPlugin.getInstance().conf().factions().landRaidControl().power()
                 .isPeacefulMembersDisablePowerLoss() && fplayer.hasFaction() && fplayer.getFaction()
                 .isPeaceful()) {
    ...
    if (!powerLossEvent.isCancelled()) {
      fplayer.alterPower(
        -FactionsPlugin.getInstance().conf().factions().landRaidControl().power().getLossPerDeath());
    }
