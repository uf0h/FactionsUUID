...
public class DTRControl implements LandRaidControl {
  ...
  public void onDeath(Player player) {
    ...
    if (!dtrLossEvent.isCancelled()) {
      faction
          .setDTR(Math.max(conf().getMinDTR(), faction.getDTR() - conf().getLossPerDeath(player.getWorld())));
  ...
  public void updateDTR(Faction faction, int minusPlayer) {
    ...
    long onlineInEnabledWorlds =
        faction.getOnlinePlayers().stream().filter(p -> plugin.worldUtil().isEnabled(p.getWorld())).count();
    double rate = Math.min(conf().getRegainPerMinuteMaxRate(),
        Math.max(0, onlineInEnabledWorlds - minusPlayer) * conf().getRegainPerMinutePerPlayer());
  ...
  public double getMaxDTR(Faction faction) {
    return Math.min(conf().getStartingDTR() + (conf().getPerPlayer() * faction.getFPlayers().size()),
        conf().getMaxDTR());
  }
